...
public class HelpMethods {
	...
	public static float GetEntityXPosNextToWall(Rectangle2D.Float hitbox, float xSpeed) {
		int currentTile_L = (int) (hitbox.x / Game.TILES_SIZE);
		int currentTile_R = (int)	((hitbox.x+hitbox.width) / Game.TILES_SIZE);
		if (xSpeed > 0) {
			// Right
			int tileXPos = currentTile_R * Game.TILES_SIZE;
			return tileXPos- 1;
		} else
			// Left
			return currentTile_L * Game.TILES_SIZE;
	}

	public static float GetEntityYPosUnderRoofOrAboveFloor(Rectangle2D.Float hitbox, float airSpeed ,int nr) {
		int currentTile = (int) (hitbox.y / Game.TILES_SIZE);
		if (airSpeed > 0) {
			// Falling - touching floor
			int tileYPos = currentTile * Game.TILES_SIZE;
			int yOffset = (int) (nr*Game.TILES_SIZE - hitbox.height);
